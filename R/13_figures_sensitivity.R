...
source("load_data_sensitivity.R")
...
p4 <- g_hes_aki_spell_full %>%
  mutate(
    ...
    d_order_2 = case_when(d_order < 3 ~ "1-2",
                          d_order > 2 ~ ">2"),
    d_order_2 = factor(d_order_2, levels = c("1-2", ">2"))
  ) %>%
  group_by(patid, spno) %>%
  arrange(epistart) %>% 
...
top_ICD <- g_hes_diagnosis_epi %>%
  ...
  mutate(
    ...
    admission_day = (epistart - admidate)
  ) %>%
  filter(d_order_2 == "1-2") %>%
  select(-epiend, -epikey) %>% 
  distinct()
...
ggarrange(p3, p5, p4, p6, p2, ncol = 2, nrow = 3, labels = c("A", "B", "C", "D", "E"))
...
cohort_eth <- aki_cohort_demograph %>%
  tabyl(ethnicity, cluster) %>%
  rename(cohort = `1`) %>%
  adorn_percentages("col") %>%
  adorn_pct_formatting(digits = 2) %>%
  adorn_ns()

cohort_eth_figure <- aki_cohort_demograph %>% 
  mutate(total = n()) %>% 
...
cohort_chap3_top15 <- chapter_read3_percentage %>% 
  ...
  mutate(row = 1:n()) %>% 
  filter(row < 16) %>% 
  mutate(chapter = paste0(chapter_read3, " ", paste0("(", desc_chapter3, ")")),
         chapter_percentage = paste0(chapter, " - ", freq)) %>% 
...
cohort_chap2_top15 <- chapter_read2_percentage %>% 
  ...
  mutate(row = 1:n()) %>% 
  filter(row < 16) %>% 
  mutate(chapter = paste0(chapter_read2, " ", paste0("(", desc_chapter2, ")")),
         chapter_percentage = paste0(chapter, " - ", freq)) %>% 
  select(cluster, row, chapter_percentage) %>%
  arrange(cluster) %>% 
  pivot_wider(names_from = cluster, values_from = chapter_percentage) %>%
...
df_ind_k6 <- ind.coord1 %>%
  mutate(
    ...
    k3 = case_when(
      k3 == 1 ~ "1. Less multimorbid",
      k3 == 2 ~ "2. Non-specific codes",
      k3 == 3 ~ "3. Multimorbid"
    ),
    k4 = case_when(
      k4 == 1 ~ "4. Female multimorbid",
      k4 == 2 ~ "2. Non-specific codes",
      k4 == 3 ~ "1. Less multimorbid",
      k4 == 4 ~ "3. CVD disease"
    ),
    k5 = case_when(
      k5 == 1 ~ "5. Female multimorbid",
      k5 == 2 ~ "3. Non-specific codes",
      k5 == 3 ~ "2. Less multimorbid",
      k5 == 4 ~ "4. CVD disease",
      k5 == 5 ~ "1. Typical AKI"
    ),
    k6 = case_when(
      k6 == 1 ~ "2. Less multimorbid (n = 40,542)",
      k6 == 2 ~ "4. Non-specific codes (n = 484)",
      k6 == 3 ~ "5. CVD disease (n = 27,751)",
      k6 == 4 ~ "3. Mental health (n = 8,177)",
      k6 == 5 ~ "1. Typical AKI (n = 43,030)",
      k6 == 6 ~ "6. Female multimorbid (n = 10,641)"
    )
  ) %>%
...
figure <- chapter_read2_percentage %>%
...
figure <- cluster_count_k %>%
  ...
  #   cluster == "1" ~ "2. Less multimorbid",
  #   cluster == "2" ~ "4. Non-specific codes",
  #   cluster == "3" ~ "5. CVD disease",
  #   cluster == "4" ~ "3. Mental health",
  #   cluster == "5" ~ "1. Typical AKI",
  #   cluster == "6" ~ "6. Female multimorbid",
  # )) %>%
  ...
  scale_y_continuous(
    expand = c(0, 0), limits=c(0,400), breaks=seq(0,400, by = 100)
  ) +
  scale_x_date(
    expand = c(0, 0),
    date_breaks = "1 year",
...
ggsave("cluster_time_series.jpg")

# sankey sensitivity analysis
# 
# ind.coord$cluster_diagnostic_position <- factor(km.res$cluster)
# 
# df_ind_k6 <- ind.coord %>% 
#   make_long(cluster, cluster_diagnostic_position)                          
# 
# df_ind_k6 %>%
#   ggplot( aes(x = x,
#               next_x = next_x,
